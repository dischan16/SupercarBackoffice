			...
			{			
				...
				lieu = droplieu.getSelectedItem();
				 ...
				 try {
					 ...
					 final String SALAIRE_REGEX = "^[0-9]{6}$";
					...
					if (NOM_PATTERN.matcher(nom).matches() && PRENOM_PATTERN.matcher(prenom).matches()
						...
						&& BANCAIRE_PATTERN.matcher(compte_bancaire).matches() && NUMID_PATTERN.matcher(numero_id).matches()
					...
					pst = con.prepareStatement("insert into rh_employee(civilite,nom,prenom,age,telephone,adresse,email,license,compte_bancaire,numero_id,salaire,poste,departement,lieu)values(?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
			...
			public void keyReleased(KeyEvent e) {
				 ...
				 try {
			            ...
			            String id = txtid_emp.getText();
			                ...
			                pst = con.prepareStatement("select civilite,nom,prenom,age,telephone,adresse,email,license,compte_bancaire,numero_id,salaire,poste,departement,lieu from rh_employee where id_emp = ?");
			                pst.setString(1, id);
		...
		btnUpdate.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				id_emp  = txtid_emp.getText();
				 ...
				 try {
						pst = con.prepareStatement("update rh_employee set civilite= ?,nom=?,prenom=?,age=?,telephone=?,adresse=?,email=?,license=?,compte_bancaire=?,numero_id=?,salaire=?,poste=?,departement=?,lieu=? where id_emp =?");
						pst.setString(1, (String) civilite);
			            pst.setString(2, nom);
			            pst.setString(3, prenom);
			            pst.setString(4, age);
